...
Present verification results directly in the conversation. Do not create or append `nathan-setup-report.md`, relocate it, or create another setup report file. Existing reports from older runs are left untouched unless the user explicitly asks to remove them. Local `.nathan-setup/` continues to store baselines and probe metadata for status and retries, ignores itself in Git and is not portable project policy. Exclude credentials and raw provider logs from the user-facing results.
