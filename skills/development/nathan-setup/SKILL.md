...
Present verification results directly in the conversation; setup writes no report file. Leave any existing `nathan-setup-report.md` untouched unless the user explicitly asks to remove it. Local `.nathan-setup/` continues to store baselines and probe metadata for status and retries, ignores itself in Git and is not portable project policy. Exclude credentials and raw provider logs from the user-facing results.
