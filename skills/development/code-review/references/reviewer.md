...
- **If a command is denied or blocked, don't route around it.** Find a read-only way to get the same information, and if there genuinely isn't one, say so in your report rather than dropping the finding silently.
