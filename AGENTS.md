...
1. Make the change, then `npm run changeset` and pick the bump:
   **patch** for wording, docs or a fix inside a skill; **minor** for a new skill
   or changed inputs/outcomes; **major** for anything that breaks an existing
   setup (a rename, a removed skill, a relink existing installs must re-run).
2. Commit the generated `.changeset/<name>.md` alongside the change.
...
5. `./scripts/relink.sh`
6. `npm run changeset` — a rename is a **major** bump; say the old and new name
