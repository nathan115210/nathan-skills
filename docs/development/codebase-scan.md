...
must be installed (`relink.sh` links every skill). The report keeps its identity
header — project and revision — because `to-tickets` reads it and it still says
what was scanned once the file is copied out.
